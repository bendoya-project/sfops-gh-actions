...
async function generateCsvFiles() {
  const queries = [
    `sf data query -r csv -q "Select Id, Username,Name,Email, IsActive, ProfileId From User WHERE IsActive=true" -w 300 -o ${targetOrg} > ${targetOrg}.csv`,
  ];
...
async function updateRecords(file, sObject, fieldsToUpdate) {
  ...
  for (const record of data) {
    ...
    const values = fieldsToUpdate
      ...
      .join(" ");
    const cmd = `sf data update record -o ${targetOrg} -s ${sObject} -v "${values}" -i "${record.Id}"`;
...
async function main() {
  ...
  let users = await readCsv(`${targetOrg}.csv`);
  ...
  await writeCsv(`Updated${targetOrg}.csv`, users);
  await updateRecords(`Updated${targetOrg}.csv`, "User", ["IsActive"]);
