...
const yargs = __nccwpck_require__(955);
const { hideBin } = __nccwpck_require__(570);
...
async function cloneAndPrepareRepository(githubRepoUrl, dirToCopy, targetDir) {
    try {
        ...
        execSync(`gh repo clone ${githubRepoUrl} ${tempDir}`,{ timeout: 1000*60*10 });
...
async function getRepoVariable(repo, variable) {
    try {
      ...
      const output = execSync(command,{ timeout: 1000*60*2 }).toString();
      output.trim();
...
const {applyExtends, cjsPlatformShim, Parser, Yargs, processArgv} = __nccwpck_require__(926)
...
const stringWidth = __nccwpck_require__(421);
const stripAnsi = __nccwpck_require__(34);
const wrap = __nccwpck_require__(154);
function ui(opts) {
    return cliui(opts, {
        stringWidth,
        stripAnsi,
...
const parser = new YargsParser({
    ...
    require: (path) => {
        if (true) {
            return __nccwpck_require__(933)(path);
        }
        else {}
    }
});
...
/******/ 	var __webpack_exports__ = __nccwpck_require__(__nccwpck_require__.s = 542);
